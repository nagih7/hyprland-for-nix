-- general.lua — migrated from general.conf (Hyprland 0.55 Lua config)
-- Injected verbatim into hyprland.lua via home-manager extraConfig.

-- MONITOR CONFIG
hl.monitor({ output = "", mode = "preferred", position = "auto", scale = 1 })

-- Touchpad / workspace swipe gestures
hl.config({
    gestures = {
...
-- Trackpad gestures (3/4 finger)
hl.gesture({ fingers = 3, direction = "swipe", action = "move" })
hl.gesture({ fingers = 3, direction = "pinch", action = "float" })
hl.gesture({ fingers = 4, direction = "horizontal", action = "workspace" })
-- NOTE: hl.gesture in 0.55 only supports actions {move,float,workspace,special,fullscreen,close};
-- it cannot run an arbitrary dispatcher, so these two (4-finger up/down -> quickshell overview)
-- have no Lua equivalent and are dropped:
...
hl.config({
    ...
    decoration = {
        ...
        shadow = {
            enabled = true,
            -- ignore_window removed: option no longer exists in 0.55
            range = 20,
            offset = "0 2",
            render_power = 10,
            color = "rgba(00000020)",
    ...
    misc = {
        ...
        -- vfr removed: option no longer exists in 0.55
        vrr = 2,
